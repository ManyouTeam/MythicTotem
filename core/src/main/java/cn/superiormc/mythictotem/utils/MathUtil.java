...
public class MathUtil {
    ...
    public static String toDisplayString(BigDecimal value) {
        ...
        BigDecimal working = ConfigManager.configManager.getBoolean("number-display.strip-trailing-zeros.enabled", true) ? value.stripTrailingZeros() : value;
        if (!ConfigManager.configManager.getBoolean("number-display.format.enabled", true)) {
    ...
    public static BigDecimal doCalculate(String mathStr, int scale) {
        try {
            if (!ConfigManager.configManager.getBoolean("math.enabled", true)) {
                return new BigDecimal(mathStr);
            }
            if (ConfigManager.configManager.getBoolean("math.enable-function", true)) {
